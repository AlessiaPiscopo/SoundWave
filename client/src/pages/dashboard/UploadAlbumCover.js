...
const UploadAlbumCover = () => {
  ...
  const [albumCoversList, setAlbumCoversList] = useState([]);
  const albumCoversListRef = ref(storage, "albums/album-covers/");

  const uploadAlbumCover = () => {
    if (newAlbumCover == null) return;

    const albumCoverRef = ref(
      storage,
      `albums/album-covers/${newAlbumCover.name + v4()}`
    );

    uploadBytes(albumCoverRef, newAlbumCover).then((snapshot) => {
      getDownloadURL(snapshot.ref).then((url) =>
        setAlbumCoversList((prev) => [...prev, url])
      );
    });
  };
  ...
  useEffect(() => {
    listAll(albumCoversListRef).then((response) => {
